...
public class treemnatimed : MonoBehaviour
{
  ...
  //  bool leftlob = false;
    ...
    void Start()
    {
       ...
       // InvokeRepeating("Triple", 3f, 6f);
        InvokeRepeating("Attack",startdelay ,attacktime );
      // Invoke("Attack",5f);
    }
     ...
     void Shoot(Transform fire)
    {
        ...
        int rad = Random.Range(1, 50);
         GameObject shotIn = Instantiate(prefabBall, fire.position, fire.rotation);
         Rigidbody rb = shotIn.GetComponent<Rigidbody>();

         rb.AddForce(fire.forward*(fireForce+rad), ForceMode.Force);
         Destroy(shotIn,8);
